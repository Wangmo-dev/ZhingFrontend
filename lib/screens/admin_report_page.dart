...
class _AdminReportPageState extends State<AdminReportPage> {
  ...
  Future<void> _loadScans() async {
    try {
      ...
      // ✅ Filter by same dzongkhag and exclude 'created' status
      final filtered =
          allScans.where((scan) {
            final user = scan['user'];
            final status = (scan['status'] ?? '').toLowerCase();
            return user != null &&
                user['dzongkhag'] == myDzongkhag &&
                status != 'created'; // Exclude 'created'
          }).toList();
  ...
  void _forwardReport(dynamic scan) {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('Report forwarded')));
  }
    ...
    if (pickedDate != null) {
      ...
      if (pickedTime != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Visit scheduled with $name for ${DateFormat.yMMMd().add_jm().format(scheduledDateTime)}',
            ),
          ),
        );
      }
